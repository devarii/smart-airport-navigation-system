  ...
  const WALL_OFFSET = {
  T1: { f1Y: 0, f2Y: -2, f1X: 0, f2X: 0},
  T2: { f1Y: 0, f2Y: 0, f1X: 0, f2X: 0 },
} as const;

  const T1_ROOM_CALIBRATION = {
    floor2: { offsetX: 12, offsetY: -24, scaleX: 1, scaleY: 1 },
    floor1: { offsetX: 0, offsetY: -4, scaleX: 1, scaleY: 1 },
  } as const;
  ...
  export default function MapCanvas({ children }: { children?: ReactNode }) {
    ...
    // ── Helper: konversi koordinat grid → koordinat visual setelah per-floor rotate ──
    const toVisualCoord = useCallback((
      x: number, y: number, floor: "l1" | "l2",
    ): { vx: number; vy: number } => {
      if (floor === "l2") {
        const pivot = (cfg.f2Max * CELL + CELL) / 2;
        return { vx: svgW - x, vy: 2 * pivot - y };
      }
      const pivot = (cfg.f1Min * CELL + (cfg.rows - 1) * CELL) / 2;
      return { vx: svgW - x, vy: 2 * pivot - y };
    }, [svgW, cfg.f2Max, cfg.f1Min, cfg.rows]);

    return (
      <div
        ref={outerRef}
        className="relative h-full w-auto max-w-full rounded-2xl overflow-visible shadow-[0_2px_24px_rgba(0,0,0,0.08)]"
          ...
          >
            ...
            {isGridMode && (
              <GridWallLayer
                ...
                offsetX={7}   offsetY={23}
                offsetXF2={-22} offsetYF2={15}
                offsetXF1={-10} offsetYF1={0}
              />
            )}
 {/* LAYER 2 + 3: Room boxes & pins — per-floor rotate dalam bounding box */}

{/* ── FLOOR 2: rotate 180° dalam bbox r=0..f2Max ── */}
{(() => {
  const cxF2 = svgW / 2;
  const cyF2 = (cfg.f2Max * CELL) / 2;
  const offF2 = WALL_OFFSET[cfg.calibKey];
  return (

    <g transform={`translate(${offF2.f2X}, ${offF2.f2Y}) rotate(180, ${cxF2}, ${cyF2})`}>
      {(cfg.destinations as DestinationWithRoom[]).map((dest, i) => {
        if (!dest.id.startsWith("l2_")) return null;

        if (dest.room && isVisible(dest)) {
          ...
          return (
            <g key={`room-${dest.id}-${i}`} role="button" tabIndex={0}
              aria-label={dest.label} aria-pressed={isSelected} style={{ cursor: "pointer" }}
              onClick={(e) => { e.stopPropagation(); const { vx, vy } = toVisualCoord(x + w / 2, y + h / 2, "l2"); handleDestinationClick(dest, vx, vy); }}
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); const { vx, vy } = toVisualCoord(x + w / 2, y + h / 2, "l2"); handleDestinationClick(dest, vx, vy); } }}
            >
              {(hitPadX > 0 || hitPadY > 0) && (
                <rect x={x - hitPadX} y={y - hitPadY} width={w + hitPadX * 2} height={h + hitPadY * 2} rx={3} fill="transparent" stroke="none" />
              )}
              <rect x={x} y={y} width={w} height={h} rx={3}
                fill={getDestColor(dest)} stroke={isSelected ? "#f39c12" : getDestColor(dest)}
                fillOpacity={isSelected ? 0.55 : 0.28} strokeWidth={isSelected ? 2.5 : 1.2} strokeOpacity={isSelected ? 1 : 0.75}
              />
              <rect x={x+1} y={y+1} width={Math.max(0,w-2)} height={Math.max(0,h-2)} rx={2}
                fill="none" stroke="white" strokeOpacity={0.3} strokeWidth={0.7} pointerEvents="none" />
              {showLabel && (
                <text
                  x={x + w / 2} y={y + h / 2}
                  ...
                  transform={`rotate(180, ${x + w / 2}, ${y + h / 2})`}
                >
                  {dest.label}
                </text>
              )}
              {isSelected && (
                <rect x={x-1} y={y-1} width={w+2} height={h+2} rx={4}
                  fill="none" stroke="#f39c12" strokeWidth={2} strokeDasharray="4 2" pointerEvents="none" />
              )}
            </g>
          );
        }
        ...
        if (!dest.room && isVisible(dest)) {
          ...
          return (
            <DestMarker key={dest.id} cx={cx} cy={cy}
              color={getDestColor(dest)} label={dest.label}
              isSelected={isSelected} onClick={() => { const { vx, vy } = toVisualCoord(cx, cy, "l2"); handleDestinationClick(dest, vx, vy); }}
            />
          );
        }
...
{/* ── FLOOR 1: rotate 180° dalam bbox r=f1Min..rows-1 ── */}
{(() => {
  const cxF1 = svgW / 2;
  const cyF1 = (cfg.f1Min * CELL + (cfg.rows - 1) * CELL) / 2;
  const offF1 = WALL_OFFSET[cfg.calibKey];
  return (
    <g transform={`translate(${offF1.f1X}, ${offF1.f1Y}) rotate(180, ${cxF1}, ${cyF1})`}>
      {(cfg.destinations as DestinationWithRoom[]).map((dest, i) => {
        if (!dest.id.startsWith("l1_")) return null;
        ...
        if (dest.room && isVisible(dest)) {
          ...
          return (
            <g key={`room-${dest.id}-${i}`} role="button" tabIndex={0}
              aria-label={dest.label} aria-pressed={isSelected} style={{ cursor: "pointer" }}
              onClick={(e) => { e.stopPropagation(); const { vx, vy } = toVisualCoord(x + w / 2, y + h / 2, "l1"); handleDestinationClick(dest, vx, vy); }}
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); const { vx, vy } = toVisualCoord(x + w / 2, y + h / 2, "l1"); handleDestinationClick(dest, vx, vy); } }}
            >
              {(hitPadX > 0 || hitPadY > 0) && (
                <rect x={x - hitPadX} y={y - hitPadY} width={w + hitPadX * 2} height={h + hitPadY * 2} rx={3} fill="transparent" stroke="none" />
              )}
        ...
        if (!dest.room && isVisible(dest)) {
          ...
          return (
            <DestMarker key={dest.id} cx={cx} cy={cy}
              color={getDestColor(dest)} label={dest.label}
              isSelected={isSelected} onClick={() => { const { vx, vy } = toVisualCoord(cx, cy, "l1"); handleDestinationClick(dest, vx, vy); }}
            />
          );
        }
...
{/* LAYER 4: Floor labels — posisi disesuaikan dengan background baru */}
<FloorLabel x={12} y={cfg.labelL1Y} label="LANTAI 1" />
<FloorLabel x={12} y={cfg.labelL2Y} label="LANTAI 2" />

{/* LAYER 5: Kiosk marker — tidak ikut rotate, tetap di koordinat grid asli */}
<KioskMarker
